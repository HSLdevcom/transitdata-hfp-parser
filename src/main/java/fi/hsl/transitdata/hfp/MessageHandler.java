...
public class MessageHandler implements IMessageHandler {
    ...
    public void handleMessage(Message received) {
        try {
            if (TransitdataSchema.hasProtobufSchema(received, TransitdataProperties.ProtobufSchema.MqttRawMessage)) {
                ...
                try {
                    if ("apc".equals(messageType)) {
                        ...
                        converted.ifPresent(passengerCountData -> sendPulsarMessage(received.getMessageId(),
                                passengerCountData, timestamp,
                                TransitdataProperties.ProtobufSchema.PassengerCount.toString(), producer));
                        ...
                        sendPulsarMessage(received.getMessageId(), converted, timestamp,
                                TransitdataProperties.ProtobufSchema.HfpData.toString(), producer);
                    }
                } catch (HfpParser.InvalidHfpTopicException | HfpParser.InvalidHfpPayloadException
                        | PassengerCountParser.InvalidAPCPayloadException invalidDataException) {
            ...
            if (e instanceof NullPointerException
                    && (e.getMessage() == null || "null".equals(e.getMessage().toLowerCase(Locale.ROOT).trim()))) {
                e.printStackTrace();
            }
        }
    }
    ...
    private Optional<PassengerCount.Data> parsePassengerCountData(Mqtt.RawMessage raw, long timestamp)
            throws IOException, PassengerCountParser.InvalidAPCPayloadException,
            PassengerCountParser.InvalidAPCTopicException {
        ...
        builder.setSchemaVersion(builder.getSchemaVersion()).setPayload(maybePayload.get()).setTopic(topic)
                .setReceivedAt(timestamp);

        return Optional.of(builder.build());
    }

    private Hfp.Data parseHfpData(Mqtt.RawMessage raw, long timestamp)
            throws IOException, HfpParser.InvalidHfpPayloadException, HfpParser.InvalidHfpTopicException {
        final String rawTopic = raw.getTopic();
        final byte[] rawPayload = raw.getPayload().toByteArray();
        ...
        builder.setSchemaVersion(builder.getSchemaVersion()).setPayload(payload).setTopic(topic);
        return builder.build();
    }

    private void sendPulsarMessage(MessageId received, GeneratedMessageV3 message, long timestamp,
            String protobufSchema, Producer<byte[]> producer) {
        producer.newMessage()
                //.key(dvjId) //TODO think about this
                .eventTime(timestamp).property(TransitdataProperties.KEY_PROTOBUF_SCHEMA, protobufSchema)
                .value(message.toByteArray()).sendAsync().whenComplete((MessageId id, Throwable t) -> {
                    if (t != null) {
